    ...
    function InstallerSection(props) {
      ...
      if (typeof props.useSessions === "function") {
        try {
          sessionId = props.useSessions(function (state) {
            const items = state && (state.items || state.sessions || state.list);
            if (Array.isArray(items) && items.length > 0) {
              const current = items.find(function (row) { return row && (row.active || row.current); }) || items[0];
              return current ? current.id : null;
            }
            return state ? (state.currentId || state.activeId || null) : null;
          });
        } catch (err) {
          sessionId = null;
        }
      ...
      React.useEffect(function () {
        ...
        Promise.resolve()
          ...
          .catch(function () { /* list failure is non-fatal */ });
