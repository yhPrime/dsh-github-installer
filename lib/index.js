...
export function apply(ctx) {
  ...
  // Desktop exposes desktopProfiles to profile bundles, but it is not
  // guaranteed to be present at bundle mount time; fall back to the standard
  // DSH_HOME layout (the same directory desktopProfiles.current.dir resolves to).
  const profile = desktopProfiles !== undefined && desktopProfiles.current !== undefined
    ? desktopProfiles.current
    : { name: 'web', dir: join(process.env.DSH_HOME || join(homedir(), '.dsh'), 'profiles', 'web') }
